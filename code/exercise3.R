...
## The data has been created under the 'Day 2: Exercise 2' script
...
state_med_colleges_sf <- state_med_colleges_sf |> 
  mutate(med_col_per_million = no_of_colleges / (state_pop_2020 / 1e6))

# Create a Choropleth of Medical Colleges per Million Population
# Plot a Choropleth
state_med_colleges_sf |> 
  ggplot() +
  geom_sf(aes(fill = med_col_per_million))
